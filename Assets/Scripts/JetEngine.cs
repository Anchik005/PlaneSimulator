...
public class JetEngine : MonoBehaviour
{
    ...
    // Плавный выход на форсаж
    private float _abLerp = 0f;
    private float _afterburnerRampRate = 0.5f; // скорость плавного набора форсажа
    ...
    private void FixedUpdate()
    {
        ...
        // Плавное увеличение тяги форсажа
        if (_afterBurner)
            _abLerp = Mathf.Clamp01(_abLerp + _afterburnerRampRate * dt);
        else
            _abLerp = Mathf.Clamp01(_abLerp - _afterburnerRampRate * dt);

        // Смешивание обычной тяги и форсажа
        float currentMaxThrust = Mathf.Lerp(_thrustDrySL, _thrustABSL, _abLerp);

        float throttle = _throttle01 * currentMaxThrust;
